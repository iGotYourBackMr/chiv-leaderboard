...
const RankTiersSidebar = ({ className = "", isMobile = false }: RankTiersSidebarProps) => {
  return (
    <aside className={`${className} bg-black/40 backdrop-blur-sm rounded-2xl border border-slate-800/50 p-4 shadow-xl`}>
      {/* Mobile layout: Grid or Stack based on screen size */}
      <div className={`${isMobile ? 'grid grid-cols-1 sm:grid-cols-2 gap-4' : ''}`}>
        ...
        <div className={`${isMobile ? 'sm:border-l sm:border-slate-800/50 sm:pl-4' : 'mt-6'}`}>
